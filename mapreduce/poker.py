...
def mapper(record):
    ...
    straightrunfaces = 'A23456789TJQKA';   # note: ace ('A') appears twice in 14-card sequence
    ...
    for i in range(0, 11):                 # 
        ...
        # Get the straights - The range to be tested is counts[0:4] /A2345 up to counts[9:13] or TJQKA
      if (i < 10):
        if (counts[straightrunfaces[i]] and 
              counts[straightrunfaces[i+1]] and 
              counts[straightrunfaces[i+2]] and 
        ...
        #  The range to be tested is counts[0:3] /A234 up to counts[10:13] or TJQKA
        if (counts[straightrunfaces[i]]   and 
              counts[straightrunfaces[i+1]] and 
              counts[straightrunfaces[i+2]] and 
              counts[straightrunfaces[i+3]] and  
              counts[straightrunfaces[i+4]] == 0 ):
          is_4straight += 1
      
      # Handle boundary condition, counts[9] == 0 and counts[10:13]
      else:
        # print i, straightrunfaces[i+3]
        if ( (counts[straightrunfaces[i-1]] == 0) and 
                counts[straightrunfaces[i]] and 
                counts[straightrunfaces[i+1]] and 
                counts[straightrunfaces[i+2]] and  
    ...
    elif is_straight:                                           # 10,200
         mr.emit_intermediate('straight', '1') 
    elif is_4straight:
          mr.emit_intermediate('4cardstraight', is_4straight)   # > 10,200 
    elif is_trip: 
